...
public class SPUM_Prefabs : MonoBehaviour
{
    ...
    public void PlayAnimation(PlayerState playState, int index, float speed = 1f)
    {
        Animator animator = _anim;
        string stateStr = playState.ToString();

        var animations = StateAnimationPairs[stateStr];
        OverrideController[stateStr] = animations[index];
        ...
        if (!isMove && !isDebuff)
        {
            foreach (var parameter in animator.parameters)
            {
                ...
                {
                    if (parameter.name.ToUpper().Contains(stateStr.ToUpper()))
                    {
